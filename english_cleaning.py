...
class EnglishCleaner:
    '''
        allows to pre-process and clean tweets from a dataframe containing dirty english tweets
    ...
    def stop(self, tweet_text_cleaned):
        '''
            cleans tokenized text from: stopwords; english_trash_words
                                                                    '''
        ...
        tweet_nostop = [t for t in tweet_text_cleaned if
                        (not t.is_punct and not t.is_stop and not t.text.lower() in english_trash_words)]
        return tweet_nostop

    def lemma(self, tweet_nostop):
        '''
            lemmatizes words, removes: stopwords and english trash words once more after lemmatization;
        ...
        tweet_text_final_splitted_no_trash = [word for word in tweet_text_final_splitted_trash if (
                word not in STOP_WORDS and word not in english_trash_words and len(word) > 1)]
